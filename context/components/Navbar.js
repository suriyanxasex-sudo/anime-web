...
import { FaSearch, FaCrown, FaUserShield, FaSignOutAlt, FaUserEdit } from 'react-icons/fa';
...
export default function Navbar() {
  ...
  const handleSearch = (e) => {
    ...
    if (search.trim()) {
      ...
      // ระบบ Auto-Clear Search เมื่อเปลี่ยนหน้า (Optional)
    }
  };
  ...
  return (
    <nav className="bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5 sticky top-0 z-[100] px-4 md:px-10 h-20 flex justify-between items-center shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
      ...
      {/* 👑 [BRANDING] - เปลี่ยนจาก AnimeJosh เป็น Jplus Manga+ */}
      ...
      {/* 🔍 [SEARCH_ENGINE] - ปรับดีไซน์ให้ดูดุและเท่ขึ้น */}
      ...
      <div className="flex gap-3 md:gap-6 items-center">
        ...
        {/* VIP_BADGE สำหรับคนยังไม่เป็น Premium */}
        {user && !user.isPremium && (
        ...
        {user ? (
          <div className="flex items-center gap-4 bg-white/5 pl-2 pr-4 py-1.5 rounded-2xl border border-white/10">
            {/* User Avatar */}
            <Link href="/profile" className="w-9 h-9 rounded-xl border-2 border-[#FB7299] overflow-hidden hover:scale-110 transition-transform">
              <img 
